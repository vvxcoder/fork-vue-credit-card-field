  ...
  watch: {
    ...
    showSecurityFields(value) {
      try {
        ...
        console.log('error line 483 = ', error);
      }
    }
  },
  created() {
    ...
    this.validated = Object.keys(this.card).reduce((carry, value) => {
      carry[value] = this.errors[value] ? false : null;

      return carry;
    }, {});
    this.currentErrors = Object.assign({number: this.error}, this.errors);
    this.showSecurityFields = !!this.card.number,
...
<style lang="scss">
@import "@/assets/styles/credit-card-field.scss";
</style>
